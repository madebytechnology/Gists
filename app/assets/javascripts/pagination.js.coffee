jQuery ->
  ...
  pushPage = (page) ->
    History.pushState null, "MyGists | Page " + page, "?page=" + page
    return
    ...
    $(window).bindWithDelay 'scroll', ->
      ...
      if more_posts_url && $(window).scrollTop() > $(document).height() - $(window).height() - 60
        $('#infinite-scrolling .pagination').html(
          '<img src="/assets/images/ajax-loader.gif" alt="Loading..." title="Loading..." />')
  ...
  if $('#with-button').size() > 0
    ...
    $('#load_more_posts').show().click ->
      unless loading_posts
        ...
        if more_posts_url
          $this = $(this)
          $this.html('<img src="/assets/images/ajax-loader.gif" alt="Loading..." title="Loading..." />').addClass('disabled')
          $.getScript more_posts_url, ->
